...
main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "dnv")) != -1) {
        ...
        default:
            usageErr("%s [options] [PID [UID [GID]]]\n"
                    "    Options:\n"
                    "\t-d    Use datagram socket (default is stream)\n"
                    "\t-n    Don't construct explicit credentials structure\n"
                    "\t-v    Be verbose\n"
                    "Each of the arguments can be '-', meaning send "
                    "process's own PID/UID/GID\n", argv[0]);
        }
    }
    ...
    if (noExplicitCreds) {
        ...
        /* Don't construct an explicit credentials structure. (It is not
           necessary to do so, if we just want the receiver to receive our
           real credentials.) */
        ...
        /* Use sender's own PID, real UID, and real GID, unless alternate
           values were supplied on the command line. */

        struct ucred creds;

        creds.pid = getpid();
        if (argc > optind && strcmp(argv[optind], "-") != 0)
            creds.pid = atoi(argv[optind]);

        creds.uid = getuid();
        if (argc > optind + 1 && strcmp(argv[optind + 1], "-") != 0)
            creds.uid = atoi(argv[optind + 1]);

        creds.gid = getgid();
        if (argc > optind + 2 && strcmp(argv[optind + 2], "-") != 0)
            creds.gid = atoi(argv[optind + 2]);
