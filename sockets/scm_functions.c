...
/* Send the file descriptor 'fd' over the connected UNIX domain socket 'sockfd'.
   If *data is not NULL, then it points to real data of length 'dataLen' that
   should be sent with the ancillary data. If *dta is NULL, then some "dummy"
   real data is sent.

   Returns 0 on success, or -1 on error. */

int
sendfd(int sockfd, int fd, void *data, size_t dataLen)
{
    ...
    /* On Linux, we must transmit at least one byte of real data in order to
       send ancillary data. We transmit the data specified by 'data' and
       'dataLen', or some dummy data if 'data' is NULL. */
...
/* Receive a file descriptor on a connected UNIX domain socket. If *data is not
   NULL, then up to 'dataLen' bytes of accompanying real data are placed in
   that buffer. If *data is NULL, then a minumal of "dummy" real data is read.

   Returns the received file descriptor on success, or -1 on error. */

int
recvfd(int sockfd, void *data, size_t dataLen)
{
